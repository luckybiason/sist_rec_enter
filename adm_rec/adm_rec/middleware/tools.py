...
class LoginEnforce(object):
    ...
    def __init__(self):
        ...
        if hasattr(settings,'PUBLIC_URLS'):
            ...
            public_links = [url for url in settings.PUBLIC_URLS]
        else:
            public_urls = [(re.compile("^%s$" % ( self.login_url[1:] )))]
            public_links = [url for url in ['/login/'] ]
    ...
    def process_request(self, request):
        ...
        """
        try:
            if request.user.is_anonymous(): # se o usuário não está logado
                for url in self.public_urls: # (apenas para o caso de URL's públicas)
                    print url, ' ', request.path[1:]
                    if url.match(request.path[1:]): #verifica se a url atual pertence a lista de públicas
                        return None # se pertence permanece nela.
