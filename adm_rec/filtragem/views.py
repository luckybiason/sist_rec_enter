...
def passo02(request):
    ...
    tvs    = request.POST.get("tvs_ids","")
    tvs_id = [ tv.split(",")[0] for tv in tvs.split(";")]
    ...
    televisores_ids = get_ids_str(classificacao_uso(televisores, usos ))
    televisores_ids =  ";".join( [ tv for tv in tvs.split(";") if tv.split(',')[0] in televisores_ids] )
...
def passo03(request):
    '''
     Passo 03: Quantidade de aparelhos. 
    '''
    ## Captura dos televisores 
...
def passo04(request):
    ...
    return render(request, "filtragem/passo05.html", locals())
